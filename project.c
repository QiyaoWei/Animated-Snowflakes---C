...
volatile bool freeze_screen = false;
...
int main(void) {
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
    ...
    while (1) {
        ...
        if (current_frame == 0 && !freeze_screen)
            generate_snowflake();

        draw_snowflakes();
        
        // poll switches
        max_num_snowflakes = *(sw);
...
void draw_snowflakes() {
	...
	srand(time(NULL));
    // gsl_rng *generator = gsl_rng_alloc(gsl_rng_rand);
    for (int i = 0; i < num_snowflakes; i++) {
		/*
        // int diff_x = snowflakes[i].x - snowflakes[i].init_x;
        double levy = gsl_ran_levy_skew(generator, 1, 0.5, levy_skew);
        int dx = (levy > 0) ? 1 : -1;
		*/
		int dx = (rand() % 2 == 0) ? -1 : 1;
        
        /*
        // approximate a normal distribution using the 68-95-99.7 rule
        if (diff_x > 0 && diff_x <= standard_deviation)
            dx = (rand() % 100 >= 50) ? 1 : -1;
        else if (diff_x > standard_deviation && diff_x <= 2*standard_deviation)
            dx = (rand() % 100 >= 84) ? 1 : -1;
        else if (diff_x > 2*standard_deviation && diff_x <= 3*standard_deviation)
            dx = (rand() % 100 >= 98) ? 1 : -1;
        else if (diff_x > 3*standard_deviation)
            dx -1;
        else if (diff_x > -standard_deviation && diff_x <= 0)
            dx = (rand() % 100 >= 50) ? 1 : 1;
        else if (diff_x > -2*standard_deviation && diff_x <= standard_deviation)
            dx = (rand() % 100 >= 16) ? 1 : -1;
        else if (diff_x > -3*standard_deviation && diff_x <= 2*standard_deviation)
            dx = (rand() % 100 >= 2) ? 1 : -1;
        else if (diff_x < -3*standard_deviation)
            dx = 1;
        */
            ...
            for (int y = 0; y < snowflake_pixel_size; y++) {
                if (snowflakes[i].pixels[y][x] == 1) {
                    if (!freeze_screen)
                        ...
                        plot_pixel(snowflakes[i].x + x, snowflakes[i].y + y, color);
                }
            }
        }
...
void clear_screen() {
    for (int x = 0; x < 320; x++) {
        for (int y = 0; y < 240; y++) {
            short int color = MYIMAGE_bin[640*y + 2*x + 4] + (MYIMAGE_bin[640*y + 2*x + 5] << 8);
